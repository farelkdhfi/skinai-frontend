...
import {
    ...
    Check, Info, ExternalLink, TrendingUp,
    Share2, Download,
    Loader2
} from 'lucide-react';
...
export default function ResultsPage() {
    ...
    const { results, patches, recommendations, saveToHistory, reset } = useAnalysis();
    ...
    const handleSave = async () => {
        if (isSaved || isSaving) return; // Mencegah double click
    ...
    return (
        <div className="min-h-screen bg-white text-zinc-950 pb-24">
            <Header />

            <motion.main
                className="max-w-5xl mx-auto px-6 pt-30 pb-12"
            ...
            >
                ...
                <motion.div variants={itemVariants} className="flex items-center justify-between mb-12">
                    ...
                    <div className="text-xs font-medium text-zinc-300 uppercase tracking-widest">
                        Analysis Complete
                    </div>
                </motion.div>
                ...
                <div className="grid lg:grid-cols-12 gap-12 mb-16">
                    ...
                    <div className="lg:col-span-5 space-y-8">
                        <motion.div variants={itemVariants} className="relative">
                            ...
                            <h1 className="text-6xl md:text-7xl font-medium tracking-tighter text-zinc-900 mb-4 leading-[0.9]">
                                {diagnosis}
                            </h1>
                        ...
                        <motion.div variants={itemVariants} className="flex items-center gap-6 pt-6 border-t border-zinc-100">
                            <div className="relative w-20 h-20">
                                {/* Simple CSS Ring Chart */}
                                <svg className="w-full h-full transform -rotate-90">
                                    <circle cx="40" cy="40" r="36" stroke="currentColor" strokeWidth="6" fill="transparent" className="text-zinc-100" />
                                    <circle
                                        cx="40" cy="40" r="36"
                                        stroke="currentColor" strokeWidth="6" fill="transparent"
                                        strokeDasharray={`${confidence * 226} 226`}
                                        className={`${colors.text.replace('text-', 'text-')}`}
                                    />
                                </svg>
                                <div className="absolute inset-0 flex items-center justify-center flex-col">
                                    <span className="text-sm font-bold text-zinc-900">{(confidence * 100).toFixed(0)}%</span>
                                </div>
                            </div>
                            <div>
                                <h3 className="font-medium text-zinc-900">Confidence Score</h3>
                                <div className="text-sm text-zinc-400 font-light flex items-center gap-1">
                        ...
                        {results.predictions?.length === 1 && results.predictions[0].region === 'full_image' ? (
                            ...
                            <motion.div variants={itemVariants}>
                                <div className="flex items-baseline justify-between mb-6">
                                    <h3 className="text-lg font-medium text-zinc-900">Regional Analysis</h3>
                                    <span className="text-xs text-zinc-400 font-light uppercase tracking-wider">AI Focus Areas</span>
                                </div>
                ...
                {recommendations && (
                    <motion.section variants={itemVariants} className="border-t border-zinc-100 pt-16">
                        <div className="flex items-center gap-3 mb-10">
                            <span className="w-1.5 h-8 bg-indigo-600 rounded-full"></span>
                            <div>
                                <h2 className="text-2xl font-medium text-zinc-900">Curated Regimen</h2>
                                <p className="text-zinc-400 font-light">Ingredients tailored to your skin profile.</p>
                            </div>
                        </div>
                ...
                <motion.div
                    ...
                    className="fixed bottom-0 left-0 right-0 p-6 bg-white/80 backdrop-blur-xl border-t border-zinc-200 z-50"
                >
                    <div className="max-w-5xl mx-auto flex flex-col sm:flex-row items-center justify-end gap-4">
                        <span className="hidden sm:block text-sm text-zinc-400 mr-auto">
                            Analysis ID: <span className="font-mono text-zinc-900">{results.analysis_id?.substring(0, 8) || '---'}</span>
                        </span>

                        <Link
                            to={ROUTES.ANALYZE}
                            className="w-full sm:w-auto px-6 py-3 rounded-xl font-medium text-zinc-600 hover:text-zinc-900 hover:bg-zinc-50 border border-transparent hover:border-zinc-200 transition-all flex items-center justify-center gap-2"
                        >
                        ...
                        {isAuthenticated && (
                            <button
                                ...
                                className={`
                                    w-full sm:w-auto px-8 py-3 rounded-xl font-medium flex items-center justify-center gap-2 transition-all shadow-lg
                                    ${isSaved
                        ...
                        {!isAuthenticated && (
                            <Link
                                to={ROUTES.LOGIN}
                                className="w-full sm:w-auto px-8 py-3 bg-indigo-600 text-white rounded-xl font-medium flex items-center justify-center gap-2 hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-200"
                            >
                                Login to Save
                            </Link>
                        )}
...
function PatchCard({ item, patch, heatmap }) {
    ...
    return (
        <div className="group relative bg-zinc-50 rounded-2xl overflow-hidden border border-zinc-100 hover:border-zinc-300 transition-all duration-300">
            <div className="aspect-square relative">
                ...
                <div className="absolute top-3 left-3">
                    <span className={`text-[10px] font-bold px-2 py-1 rounded-full bg-white/90 backdrop-blur shadow-sm ${colors.text}`}>
...
function SingleImageCard({ item, patch, heatmap }) {
    ...
    return (
        <div className="group relative bg-zinc-50 rounded-3xl overflow-hidden border border-zinc-200 shadow-sm">
            ...
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 px-4 py-2 bg-white/90 backdrop-blur-md rounded-full shadow-lg border border-white/50 flex items-center gap-2">
                <span className="text-xs font-medium text-zinc-500">Prediction:</span>
...
function IngredientRow({ ingredient, type = 'primary' }) {
    ...
    return (
        ...
        `}>
            <div className={`mt-0.5 w-6 h-6 rounded-full flex items-center justify-center shrink-0 ${isPrimary ? 'bg-indigo-50 text-indigo-600' : 'bg-zinc-200 text-zinc-500'}`}>
                <Check className="w-3.5 h-3.5" strokeWidth={3} />
            </div>

            <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                    <h4 className="font-medium text-zinc-900">{ingredient.name}</h4>
